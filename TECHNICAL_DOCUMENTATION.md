...
When a `GEMINI_API_KEY` is present in the environment, all causal summaries, anomaly interpretations, and chat responses are generated by Gemini via the `/v1beta/models/gemini-2.0-flash:generateContent` endpoint. The LLM receives a structured analytics context payload containing:
