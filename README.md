...
**Does it actually beat a naive guess?** Mixed, and fold-count dependent — verified against a rolling-origin backtest, not asserted. On the standard 3-fold backtest, ROAS beats a naive trailing-30-day baseline at all three horizons (16–29% improvement); Revenue beats it at 60 and 90 days (+5.5% / +6.9%) and trails narrowly at 30 days (-5.1%), a gap we tried twice to close and disclosed rather than hid when we couldn't. Under a harder 12-fold stress test (more rolling origins, same data), Revenue's edge over naive widens further (+42–46% across all three horizons), but ROAS's reverses: it loses to the naive baseline at 30 and 60 days (-40% / -43%) and only narrowly wins at 90 days (+3.7%) — we think because ROAS's low-variance denominator makes it more sensitive to the sparser/harder folds than Revenue is. We're reporting both fold counts rather than only the friendlier 3-fold number. Full breakdown, including a real bug we found and fixed in January-origin forecasts, is in [Rolling-Origin Backtesting](#-technical-documentation) below.
...
**A note on interval coverage and fold-count sensitivity.** Interval coverage reported from a 3-fold backtest is optimistic: it's computed from just 3 evenly-spaced rolling origins, and re-running the identical model and data with `--folds 8` (more origins, same historical data — a stress test, not new data) showed coverage collapsing from ~91% to 62.3%, because the wider set of origins includes sparser, harder periods (early history, the Nov–Dec demand-spike window) the 3-fold sample happened to skip. The original interval scale (`1.45x`) was tuned against only those 3 folds, which is exactly the kind of overfitting risk this project's own methodology warns against elsewhere (see the Revenue-vs-naive discussion above). We re-tuned the scale empirically against the 8-fold backtest instead and settled on `3.0x`, which brings 8-fold coverage to 77.0% — a real, verified improvement, though not a claim of hitting 90% coverage under harder conditions. The remaining gap traces to a P10 floor in the code (`max(5% of P50, computed P10)`) that caps how much further widening can help; closing it fully would need a separate fix beyond this pass. Point estimates (WAPE, ROAS/Revenue MAPE) are unaffected by this change, since the interval scale only widens P10/P90, not the P50 forecast itself.
